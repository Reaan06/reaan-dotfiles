...
def normalize_name(cls_name, title=""):
    ...
    if any(x in name for x in ["sober", "roblox", "vinegar"]): return "sober"
...
def load_data():
    ...
    try:
        with open(CACHE_FILE, 'r') as f:
            data = json.load(f)
            return data if "daily" in data else {"daily": {}, "weekly": {}, "monthly": {}, "_day": "", "_week": "", "_month": ""}
    except:
        return {"daily": {}, "weekly": {}, "monthly": {}, "_day": "", "_week": "", "_month": ""}
    ...
    def check_resets(self):
        ...
        with self.lock:
            if self.store.get("_day") != keys["day"]:
                self.store["daily"] = {}; self.store["_day"] = keys["day"]; changed = True
            if self.store.get("_week") != keys["week"]:
                self.store["weekly"] = {}; self.store["_week"] = keys["week"]; changed = True
            if self.store.get("_month") != keys["month"]:
                self.store["monthly"] = {}; self.store["_month"] = keys["month"]; changed = True
        return changed
    ...
    def update_stats(self):
        ...
        with self.lock:
            ...
            if now - self.last_pkg_check > 600:
                ...
                self.store["_pacman_count"] = p; self.store["_yay_count"] = y
